...
bool ML_Init() {
    ...
    auto printTensor = [](const TfLiteTensor* t, const char* name) {
    Serial.printf("[ML] %s type=%d dims=%d [", name, (int)t->type, t->dims ? t->dims->size : -1);
    if (t->dims) {
        for (int i=0; i<t->dims->size; i++) {
        Serial.printf("%d%s", t->dims->data[i], (i==t->dims->size-1) ? "" : ",");
        }
    }
    Serial.printf("] bytes=%d scale=%f zero_point=%d\n",
                    (int)t->bytes,
                    (t->params.scale),
                    (int)t->params.zero_point);
    };
